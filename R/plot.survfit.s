"plot.survfit" <-
    function (x, conf.int, mark.time = T, mark = 3, col = 1, lty = 1, 
              lwd = 1, cex = 1, log = F, xscale = 1, yscale = 1, firstx = 0, 
    ...
    if (!missing(fun)) {
        if (is.character(fun)) {
            tfun <- switch(fun, log = function(x) x, event = function(x) 1 - 
                           x, cumhaz = function(x) -log(x), cloglog = function(x) {xx<-log(-log(x)); ifelse(is.finite(xx),xx,NA)}, 
        ...
        if (is.matrix(ssurv)) {
            for (k in 1:ncol(ssurv)) {
                ...
                if (is.numeric(mark.time)) {
                    ...
                    points(xx[deaths == 0], yy[deaths == 0], pch = mark[i], 
                           col = col[i], cex = cex)
                xend <- c(xend, max(xx))
                yend <- c(yend, min(yy))
